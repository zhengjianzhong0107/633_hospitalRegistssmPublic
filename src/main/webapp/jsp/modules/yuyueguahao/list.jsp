...
<body>
		...
		<div id="content">
				...
				<div class="container mt-0">
					<div class="row breadcrumb-bar">
						<div class="col-md-6">
							<h3 class="block-title">预约挂号管理</h3>
						</div>
						<div class="col-md-6">
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item">预约挂号管理</li>
								<li class="breadcrumb-item active">预约挂号列表</li>
							</ol>
						</div>
					</div>
				</div>
				...
				<div class="row">
					...
					<div class="col-md-12">
						<div class="widget-area-2 lochana-box-shadow">
							<h3 class="widget-title">预约挂号列表</h3>							
							<div class="table-responsive mb-3">
								<div class="col-sm-12">
																																									<label>
												预约编号
												<input type="text" id="yuyuebianhaoSearch" class="form-control form-control-sm" placeholder="" aria-controls="tableId">
											</label>
																																																														<label>
												预约名称
												<input type="text" id="yuyuemingchengSearch" class="form-control form-control-sm" placeholder="" aria-controls="tableId">
											</label>
																																																																																	<label>
												医生姓名
												...
												<div class="custom-control custom-checkbox">
													...
													<label class="custom-control-label" for="select-all"></label>
																																						...
																																						<th onclick="sort('yuyuebianhao')">预约编号<i id="yuyuebianhaoIcon" class="fa fa-sort"></i></th>
																																																			<th onclick="sort('yuyuemingcheng')">预约名称<i id="yuyuemingchengIcon" class="fa fa-sort"></i></th>
																													...
																													<th>头像</th>
																																																																	<th onclick="sort('yuyueshijian')">预约时间<i id="yuyueshijianIcon" class="fa fa-sort"></i></th>
																																																			<th onclick="sort('guahaojiage')">挂号价格<i id="guahaojiageIcon" class="fa fa-sort"></i></th>
																																																			<th onclick="sort('yuyueneirong')">预约内容<i id="yuyueneirongIcon" class="fa fa-sort"></i></th>
	...
	<script>
		...
		function search(){
			searchForm = { key: ""};
													if($('#yuyuebianhaoSearch').val() != null && $('#yuyuebianhaoSearch').val() != ''){
					searchForm.yuyuebianhao ="%" +  $('#yuyuebianhaoSearch').val() + "%";
				}
				
																				if($('#yuyuemingchengSearch').val() != null && $('#yuyuemingchengSearch').val() != ''){
					searchForm.yuyuemingcheng ="%" +  $('#yuyuemingchengSearch').val() + "%";
				}
			...
			http("yuyueguahao/page","GET",{
				...
				order: sortOrder,
																						yuyuebianhao : searchForm.yuyuebianhao,	
																																	yuyuemingcheng : searchForm.yuyuemingcheng,	
		...
		function setDataRow(item,number){
			...
			row.appendChild(checkbox)
							...
							var yuyuebianhaoCell  = document.createElement('td');
									...
									row.appendChild(yuyuebianhaoCell);
				            				var yuyuemingchengCell  = document.createElement('td');

												 yuyuemingchengCell.innerHTML = item.yuyuemingcheng;

				
									row.appendChild(yuyuemingchengCell);
				  ...
				  touxiangCell.appendChild(touxiangImg);
									...
									row.appendChild(touxiangCell);
				            				var yuyueshijianCell  = document.createElement('td');

												  yuyueshijianCell.innerHTML = item.yuyueshijian;

				
									row.appendChild(yuyueshijianCell);
				            				var guahaojiageCell  = document.createElement('td');

												 guahaojiageCell.innerHTML = item.guahaojiage;

				
									row.appendChild(guahaojiageCell);
				            				var yuyueneirongCell  = document.createElement('td');

												  //如果图文详情包含图片
				  if(item.yuyueneirong && item.yuyueneirong.indexOf('img') != -1){
					//暂时只考虑图片+文字 和 文字+图片的情况
					var beginIndex = item.yuyueneirong.indexOf('<img');
					var endIndex = item.yuyueneirong.indexOf('>');				
					yuyueneirongCell.innerHTML = item.yuyueneirong.substring(beginIndex, endIndex+1).replace("img","img width='100' height='100'");				
				  }else{
				  	 if(item.yuyueneirong != null && item.yuyueneirong != "" && item.yuyueneirong.length >= 11){
				  		yuyueneirongCell.innerHTML = item.yuyueneirong.substring(0, 10) + "..."; 
				  	}else{
				  		yuyueneirongCell.innerHTML = item.yuyueneirong;
				 	}
				  }
				  

                
									row.appendChild(yuyueneirongCell);
            ...
            crossBtn1.innerHTML = "添加病例"
			crossBtn1.setAttribute("type","button");
			crossBtn1.setAttribute("class","btn btn-primary btn-sm");
			crossBtn1.setAttribute("onclick","crossTable(" + item.id + ",\"yonghubingli\")");
			if(crossBtnControl('添加病例') == true){
		...
		function sh(){
			...
			httpJson("yuyueguahao/update","POST",data,(res)=>{
				if(res.code == 0){
        ...
        function remove(id) { 
            ...
            if (mymessage == true) {
				...
				httpJson("yuyueguahao/delete","POST",paramArray,(res)=>{
					if(res.code == 0){
						getDataList();		
						alert('删除成功');
					}
		...
		function crossTable(id,crossTableName){
			...
			window.sessionStorage.setItem('crossTableName',"yuyueguahao");
        ...
        function pay(id){
			...
			window.sessionStorage.setItem('tableName',"yuyueguahao");
