            ...
            <div class="row">
                ...
                <div class="col-md-12">
                    <div class="widget-area-2 lochana-box-shadow">
                        <h3 class="widget-title">用户列表</h3>
                        <div class="table-responsive mb-3">
                            <div class="col-sm-12">
                                <label>
                                    用户名
                                    <input type="text" id="yonghumingSearch" class="form-control form-control-sm"
                                           placeholder="" aria-controls="tableId">
                                </label>
                                <label>
                                    姓名
                                    <input type="text" id="xingmingSearch" class="form-control form-control-sm"
                                           placeholder="" aria-controls="tableId">
                                </label>
                            ...
                            <table id="tableId" class="table table-bordered table-striped">
                                ...
                                <tr>
                                    ...
                                    <th onclick="sort('yonghuming')">用户名<i id="yonghumingIcon" class="fa fa-sort"></i>
                                    </th>
                                    ...
                                    <th onclick="sort('blacklist')">是否拉黑<i id="blacklistIcon" class="fa fa-sort"></i></th>
                                    <th>操作</th>
                                </tr>
                                </thead>
                                <tbody>
...
<script>
    ...
    function setDataRow(item, number) {
        ...
        blacklistCell.innerHTML = item.blacklist==0?"未拉黑":"已拉黑";
        ...
        var detailBtn = document.createElement('button');
        var detailAttr = "blacklist(" + item.id + ')';
        detailBtn.setAttribute("type", "button");
        detailBtn.setAttribute("class", "btn btn-info btn-sm 删除");
        detailBtn.setAttribute("onclick", detailAttr);
        detailBtn.innerHTML = "拉黑"
        btnGroup.appendChild(detailBtn)

        var detailBtn = document.createElement('button');
        var detailAttr = "qxblacklist(" + item.id + ')';
        detailBtn.setAttribute("type", "button");
        detailBtn.setAttribute("class", "btn btn-info btn-sm 删除");
        detailBtn.setAttribute("onclick", detailAttr);
        detailBtn.innerHTML = "取消拉黑"
        btnGroup.appendChild(detailBtn)
    ...
    function blacklist(id) {
        var mymessage = confirm("真的要拉黑吗？");
        if (mymessage == true) {
            paramArray = id;
            httpJson("yonghu/blacklist", "POST", paramArray, (res) => {
                if(res.code == 0){
                getDataList();
                alert('取消成功');
                }
    ...
    // 删除
    function qxblacklist(id) {
        var mymessage = confirm("真的要取消拉黑吗？");
        if (mymessage == true) {
                paramArray = id;
            httpJson("yonghu/qxblacklist", "POST", paramArray, (res) => {
                if(res.code == 0
        )
            {
                getDataList();
    ...
    function crossTable(id, crossTableName) {
        ...
        window.sessionStorage.setItem('crossTableName', "yonghu");
