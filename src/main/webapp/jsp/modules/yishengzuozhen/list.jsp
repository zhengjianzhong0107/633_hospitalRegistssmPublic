...
<body>
		...
		<div id="content">
				...
				<div class="container mt-0">
					<div class="row breadcrumb-bar">
						<div class="col-md-6">
							<h3 class="block-title">医生坐诊管理</h3>
						</div>
						<div class="col-md-6">
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item active">医生坐诊列表</li>
							</ol>
						</div>
					</div>
				</div>
				...
				<div class="row">
					...
					<div class="col-md-12">
						<div class="widget-area-2 lochana-box-shadow">
							<h3 class="widget-title">医生坐诊列表</h3>							
												...
												<div class="custom-control custom-checkbox">
													...
													<label class="custom-control-label" for="select-all"></label>
																													...
																													<th>头像</th>
																																																			...
																																																			<th onclick="sort('shanzhanglingyu')">擅长领域<i id="shanzhanglingyuIcon" class="fa fa-sort"></i></th>
																																																			<th onclick="sort('zuozhenshijian')">坐诊时间<i id="zuozhenshijianIcon" class="fa fa-sort"></i></th>
																																																			<th onclick="sort('guahaojiage')">挂号价格<i id="guahaojiageIcon" class="fa fa-sort"></i></th>
																																																			<th onclick="sort('gerenjieshao')">个人介绍<i id="gerenjieshaoIcon" class="fa fa-sort"></i></th>
																																					<th>操作</th>
										</tr>
									</thead>
									<tbody>
	...
	<script>
		...
		function setDataRow(item,number){
			...
			row.appendChild(checkbox)
				  ...
				  touxiangCell.appendChild(touxiangImg);
									...
									row.appendChild(youxiangCell);
				            				var shanzhanglingyuCell  = document.createElement('td');

												 shanzhanglingyuCell.innerHTML = item.shanzhanglingyu;

				
									row.appendChild(shanzhanglingyuCell);
				            				var zuozhenshijianCell  = document.createElement('td');

												 zuozhenshijianCell.innerHTML = item.zuozhenshijian;

				
									row.appendChild(zuozhenshijianCell);
									...
									row.appendChild(guahaojiageCell);
				            				var gerenjieshaoCell  = document.createElement('td');

												  //如果图文详情包含图片
				  if(item.gerenjieshao && item.gerenjieshao.indexOf('img') != -1){
					//暂时只考虑图片+文字 和 文字+图片的情况
					var beginIndex = item.gerenjieshao.indexOf('<img');
					var endIndex = item.gerenjieshao.indexOf('>');				
					gerenjieshaoCell.innerHTML = item.gerenjieshao.substring(beginIndex, endIndex+1).replace("img","img width='100' height='100'");				
				  }else{
				  	 if(item.gerenjieshao != null && item.gerenjieshao != "" && item.gerenjieshao.length >= 11){
				  		gerenjieshaoCell.innerHTML = item.gerenjieshao.substring(0, 10) + "..."; 
				  	}else{
				  		gerenjieshaoCell.innerHTML = item.gerenjieshao;
				 	}
				  }
				  

                
									row.appendChild(gerenjieshaoCell);
            ...
            crossBtn1.innerHTML = "预约挂号"
			crossBtn1.setAttribute("type","button");
			crossBtn1.setAttribute("class","btn btn-primary btn-sm");
			crossBtn1.setAttribute("onclick","crossTable(" + item.id + ",\"yuyueguahao\")");
			if(crossBtnControl('预约挂号') == true){
        ...
        function remove(id) { 
            ...
            if (mymessage == true) {
				...
				httpJson("yishengzuozhen/delete","POST",paramArray,(res)=>{
					if(res.code == 0){
						getDataList();		
						alert('删除成功');
					}
		...
		function crossTable(id,crossTableName){
			...
			window.sessionStorage.setItem('crossTableName',"yishengzuozhen");
